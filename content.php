	...
	<header class="entry-header">
		...
		<?php if ( has_post_thumbnail() && ( ! has_post_format() || has_post_format( 'aside' ) || has_post_format( 'image' ) || has_post_format( 'gallery' ) ) ) : ?>
			<a class="post-thumbnail" href="<?php echo esc_url( get_permalink() ); ?>">
				...
				?>
				<?php if ( is_sticky() ) : ?>
					<span class="sticky-post"><span class="genericon genericon-pinned"><span class="screen-reader-text"><?php _e( 'Sticky post', 'tvhwdsb' ); ?></span></span></span>
				<?php endif; ?>
			</a>
		<?php endif; ?>
	...
	<div class="entry-summary">
	    <?php
			...
			the_content( sprintf(
				...
				wp_kses( __( 'Continue reading %s', 'tvhwdsb' ), array( 'span' => array( 'class' => array() ) ) ),
				the_title( '<span class="screen-reader-text">"', '"</span>', false )
			) );
	        } else {
			if ( 'vp_video' === get_post_type() ) {
				hwdsb_vp_the_video_metadata();
			}
